...
const Dashboard = () => {
    ...
    const [tasks, setTasks] = React.useState([
        ...
        { id: 3, title: "Review deal notes & attachments", tag: "Enterprises", tagColor: "bg-indigo-50 text-indigo-700", completed: false }
    ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Today's Tasks - Width approx 30-35% */}
        <div className="lg:col-span-4 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              Today's Tasks <span className="bg-gray-200 text-gray-600 text-xs px-1.5 py-0.5 rounded-full">{tasks.filter(t => !t.completed).length}</span>
            </h3>
            <a href="#" className="text-sm text-blue-600 hover:text-blue-800 font-medium">See all</a>
          </div>
        ...
        {/* Active Deals - Width approx 65-70% */}
        <div className="lg:col-span-8 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
           <div className="flex items-center justify-between mb-6">
            ...
            <button className="flex items-center gap-1 text-sm bg-gray-50 hover:bg-gray-100 px-3 py-1.5 rounded-md text-gray-600">
      ...
      {/* Bottom Row: Recent Activity, Marketing Performance, AI Suggestions */}
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
             ...
             <div className="space-y-6">
                <ActivityItem 
                    ...
                    name="Chris Daniel"
                    action="Worksheet updated with latest report"
                    initials="CD"
                ...
                />
                 <ActivityItem 
                    ...
                    name="Nisha"
                    action="Followed up with John (2 hrs)"
                    initials="N"
                    bgColor="bg-teal-600"
                />
                 <ActivityItem 
                    ...
                    name="Madesh"
                    action="Associate has designated a evaluation"
                    initials="M"
                    bgColor="bg-blue-500"
                />
             </div>
             ...
             <div className="flex-1 relative mt-4 h-40 w-full">
                ...
                <div className="absolute top-0 right-1/2 translate-x-1/2 -mt-2 bg-white shadow-md p-2 rounded-lg z-10 border border-gray-50">
                    ...
                    <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-[10px] text-green-500 font-medium">+3.4%</span>
                    </div>
                </div>

                 {/* Wave SVG */}
                 <svg viewBox="0 0 100 40" className="w-full h-full text-indigo-500 opacity-80" preserveAspectRatio="none">
                    <defs>
                    ...
                    <path d="M0 35 C 10 35, 10 20, 20 20 C 30 20, 30 30, 40 30 C 50 30, 50 15, 60 25 C 70 35, 70 5, 80 15 C 90 25, 90 20, 100 20 V 40 H 0 Z" fill="url(#gradient)" stroke="none" />
                    <path d="M0 35 C 10 35, 10 20, 20 20 C 30 20, 30 30, 40 30 C 50 30, 50 15, 60 25 C 70 35, 70 5, 80 15 C 90 25, 90 20, 100 20" stroke="#818cf8" strokeWidth="0.8" fill="none" />
                 </svg>
             </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
             ...
             <div className="space-y-4">
               <SuggestionItem 
                 ...
                 text="3 deals likely too close this week"
                 seed="Felix"
               />
               <SuggestionItem 
                 iconColor="bg-teal-600"
                 text="2 leads are at risk - no follow - up for 5 days"
                 seed="Aneka"
               />
               <SuggestionItem 
                 iconColor="bg-blue-500"
                 text="Suggested task: Follow-up with Sarah"
                 subText="(35% close probability)"
                 seed="Jude"
               />
             </div>
          </div>
...
const StatCard = ({ label, value, subValue, subValueColor, onClick }) => (
  ...
  >
    ...
    <div className="flex items-end gap-2">
      ...
      {subValue && (
        <div className={`text-xs font-bold mb-1 ${subValueColor}`}>{subValue}</div>
      )}
    </div>
  </div>
);

const TaskItem = ({ title, tag, tagColor, completed, onToggle }) => (
  <div className="flex items-start gap-3 cursor-pointer group" onClick={onToggle}>
    <div className="mt-1">
        <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${completed ? 'bg-blue-600 border-blue-600' : 'border-gray-300 group-hover:border-blue-500'}`}>
    ...
    <div className={`flex-1 transition-opacity ${completed ? 'opacity-50' : ''}`}>
      <div className={`text-sm font-medium text-gray-800 mb-1 ${completed ? 'line-through text-gray-500' : ''}`}>{title}</div>
      <span className={`text-xs px-2 py-0.5 rounded ${tagColor}`}>{tag}</span>
    </div>
  </div>
);

const DealRow = ({ avatar, name, email, task, date, revenue, status, statusColor }) => (
    <tr className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
        ...
        <td className="py-4 text-sm text-gray-700 max-w-[150px] truncate">{task}</td>
        <td className="py-4 text-sm text-gray-700">{date}</td>
        <td className="py-4 text-sm text-gray-700 font-medium">{revenue}</td>
        <td className="py-4">
             <span className={`text-xs px-2.5 py-1 rounded-md font-medium ${statusColor}`}>
                {status}
            </span>
        </td>
    </tr>
...
const ActivityItem = ({ avatar, name, action, initials, bgColor }) => (
    <div className="flex items-start gap-3">
        <div className={`w-8 h-8 rounded-lg ${bgColor} flex items-center justify-center shrink-0`}>
             {/* Using API for avatar but fallback to initials/color block styling from image */}
...
const SuggestionItem = ({ iconColor, text, subText, seed }) => (
    <div className="flex items-start gap-3">
        <div className={`w-8 h-8 rounded-lg overflow-hidden shrink-0 ${iconColor}`}>
             <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${seed}`} alt="AI" />
        </div>
        <div>
            <div className="text-sm font-medium text-gray-800 leading-snug">{text}</div>
